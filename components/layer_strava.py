...
class StravaLayer(Layer):
    ...
    def load_data(self):
        ...
        for activity in activities:
            if activity["type"] not in {"Run", "Walk", "Yoga"}:
                continue
            ...
            day_of_year = (projected_date -
                           date(self.config.year, 1, 1)).days + 1
            ...
            self.run_data.append({
                ...
                "end_time": start_datetime.hour + start_datetime.minute / 60 + activity["elapsed_time"] / 3600,
    ...
    def plot(self, ax, base):
        ...
        for activity in self.run_data:
            ...
            # Line length is proportional to duration (in hours)
            r_end = activity["end_time"] / 24
            # Line length is proportional to distance (in km). Uncomment if needed.
            # r_end = r_start + activity["distance"] * activity_scale
            ax.plot([theta, theta], [r_start, r_end],
                    color="red" if activity["type"] == "Run" else (
                        "grey" if activity["type"] == "Yoga" else "blue"),
        ...
        for year in sorted(set(a["year"] for a in self.run_data)):
            ...
            for activity in filter(lambda x: x["year"] == year, self.run_data):
                ...
                theta_points.append(
                    activity["day_of_year"] / base.num_points * 2 * np.pi)
                r_points.append(base.start_time / 24 +
                                cumulative * km_to_radial)

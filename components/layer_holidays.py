...
class HolidaysLayer(Layer):
    ...
    def plot(self, ax: plt.Axes, base: BaseCalendarPlotter):
        ...
        for holiday in self.holidays:
            ...
            day_of_year = (holiday['date'] - date(base.year, 1, 1)).days + 1
            angle = day_of_year / base.config.days_in_year * 2 * np.pi
            
            month_idx = next((i for i, total in enumerate(cum_days) if day_of_year <= total), 11)
            month_day = day_of_year - (cum_days[month_idx - 1] if month_idx > 0 else 0)

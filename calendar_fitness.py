"""Dawn Calendar Plotter - Generates a circular calendar visualization combining dawn twilight times, weather and fitness data."""
...
def main():
    try:
        ...
        config.file_name = f"{config.city_name}_Fitness"
        config.running_target = 1000 # Yearly target for running in km

        config.strava_footer_height = 0.05  # Adjust height as needed
        config.strava_footer_offset = -0.015  # Positive values move the footer up

...
class DawnCalendarPlotter:
    ...
    def smooth_data(data: List[float], num_points: int) -> np.ndarray:
        ...
        fft_coeffs[int(len(fft_coeffs) * 0.1):] = 0  # Keep only lower frequencies
    ...
    def add_month_labels(self, ax: plt.Axes, days_in_month: List[int]) -> None:
        ...
        month_ticks = [(cumulative_days[i - 1] if i > 0 else 0) + days_in_month[i] / 2 
                      for i in range(12)]
        month_ticks_rad = [tick / self.num_points * 2 * np.pi for tick in month_ticks]
    ...
    def add_sunday_labels(self, ax: plt.Axes, days_in_month: List[int]) -> None:
        ...
        first_sunday = self.find_first_sunday(self.city.year)  # Calculate dynamically
